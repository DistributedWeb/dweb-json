    ...
    datjson.read(function (err) {
      ...
      datjson.create({ name: 'test' }, function (err) {
        ...
        datjson.read(function (err, data) {
          ...
          t.same(data.url, `dweb://${archive.key.toString('hex')}`)
...
test('Write dweb.json to archive', function (t) {
  var archive = dwebfs(ram)
  archive.ready(function () {
    var datjson = datJSON(archive)
    datjson.create(function (err) {
      ...
      function check (err) {
        ...
        datjson.read(function (err, data) {
          t.error(err, 'no error')
          t.ok(data, 'has metadata')
          t.same(data.url, `dweb://${archive.key.toString('hex')}`, 'url ok')
...
test('Write dweb.json to file and archive', function (t) {
  var archive = dwebfs(ram)
    ...
    datjson.create(function (err) {
      ...
      function checkFile (err) {
        ...
        fs.readFile(file, 'utf-8', function (err, data) {
          ...
          t.same(data.url, `dweb://${archive.key.toString('hex')}`, 'fs url ok')
